...
export const createUsersRoutes = async (fastify: FastifyInstance) => {
	fastify.get(
		Routes.USERS.GET.BASE,
		{
			onRequest: authHook,
			schema: {
				description: "Get user information (for authenticated user)",
				response: Routes.USERS.GET.RESPONSE,
				...createPathFields("user", true, "application/json")
			}
		},
		async (_req, res) => {
	...
	fastify.get<{ Querystring: GetUsersQueryStringType }>(
		Routes.USERS.GET_ALL.BASE,
		{
			onRequest: authHook,
			preHandler: roleGuardHook(Routes.USERS.GET_ALL.ROLES),
			schema: {
				description: "Get all registered users information (only for admin users)",
	...
	fastify.put<{ Body: UpdateUserRequestType }>(
		Routes.USERS.UPDATE.BASE,
		{
			onRequest: authHook,
			schema: {
	...
	fastify.put(
		Routes.USERS.UPDATE_PICTURE.BASE,
		{
			onRequest: authHook,
			schema: {
				description: "Update user profile picture",
				response: Routes.USERS.UPDATE_PICTURE.RESPONSE,
				...createPathFields("user", true, "multipart/form-data")
			}
		},
		async (req, res) => {
			if (!req.isMultipart()) {
